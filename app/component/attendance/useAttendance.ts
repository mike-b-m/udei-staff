...
// Internal alias — attendance_day rows are used as "summary" rows in this hook
type DayRow = AttendanceDay
...
const mergedData = statusRows
  ...
  // ── Step D: Sort the final merged data by student's last name ──────────
  .sort((a: any, b: any) => {
  ...
  const updateAttendanceStatus = useCallback(
    ...
    // Optimistic update + stats recompute
    setRows((prev) => {
...
const { error } = await supabase
  .from('attendance_record')
  .upsert(
    { student_id: studentId, day_id: dayId, status },
