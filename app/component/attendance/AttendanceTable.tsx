  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-indigo-700 text-white">
              ...
              <th className="sticky left-8 z-20 bg-indigo-700 px-4 py-3 text-left font-semibold min-w-[90px] text-xs">ID</th>
              <th className="px-4 py-3 text-left font-semibold min-w-[120px] text-xs">Last Name</th>
              <th className="px-4 py-3 text-left font-semibold min-w-[100px] text-xs">First Name</th>
              <th className="px-4 py-3 text-left font-semibold min-w-[80px] text-xs">Faculty</th>
              {days.map((day) => (
                <th
                  key={day.id}
                  className="px-2 py-3 text-center font-semibold min-w-[54px] text-xs"
            ...
            {filteredRows.map((row, idx) => (
              <tr key={row.student_id} className="hover:bg-slate-50 transition-colors">
                ...
                {days.map((day) => {
                  ...
                  const isPast = new Date(day.date_day) < new Date(new Date().toDateString())
      ...
      <div className="px-5 py-3 bg-slate-50 border-t border-slate-100 text-xs text-slate-400">
        💡 Scroll right to see all days. Blue ring = confirmed record (locked).
      </div>
    </div>
  )
}
