...
export const calculateStats = (records: any[]): AttendanceStats => {
  ...
  records.forEach((record) => {
    ...
    const status = record.status as keyof typeof stats.by_day[string]
    ...
    <body>
      ...
      <table>
        ...
        ${rows
            .map(
              (row, i) => {
                // Collect all day statuses for this student
                const dayStatuses = Object.entries(row)
                  .filter(([k]) => k.startsWith('S') && k.includes('D'))
                  .map(([, v]: [string, any]) => v?.status ?? 'absent')
                const overallStatus =
                  dayStatuses.length === 0
                    ? 'absent'
                    : dayStatuses.every((s) => s === 'present')
                    ? 'present'
                    : dayStatuses.some((s) => s === 'present')
                    ? 'present'
                    : 'absent'
                return `
            <tr>
              ...
              <td>${row.faculty}</td>
              <td class="status-${overallStatus}">${overallStatus === 'present' ? 'Present' : 'Absent'}</td>
            </tr>
          `
              }
            )
