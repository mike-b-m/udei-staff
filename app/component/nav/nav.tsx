...
export default function Nav(){
    ...
    <ul className=" h-[80%]">
        {/* home section nav */}
       {role.role ==='admin' ? <li className={`p-1 m-2  justify-center text-center rounded-2xl flex ${pathname === '/' ? 'bg-gray-300 border-b-2 border-green-600 text- rounded-2xl':''} `}>
           <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
  <path strokeLinecap="round" strokeLinejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
</svg>
<Link className="pl-3" href="/">Home</Link></li> :null}
            
       {role.role === 'admin' || role.role === 'editor' ?     <li className={`p-1 m-2  justify-center text-center rounded-2xl flex ${pathname === '/inscription' ? 'bg-gray-300 border-b-2 border-green-600 text- rounded-2xl':''} `}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
...
<Link className="pl-3" href="/inscription">Inscription</Link></li> : null}
       ...
       {role.role === 'admin' || role.role === 'editor' ?   <li className={`p-1 m-2  justify-center text-center rounded-2xl flex ${pathname === '/search' ? 'bg-gray-300 border-b-2 border-green-600 text- rounded-2xl':''} `}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
  <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
</svg> <Link className="pl-3" href="search">Search</Link></li> :null}
       
        {/*search section */}
        {role.role === 'admin' || role.role === 'admistration' ? <li className={`p-1 m-2  justify-center text-center rounded-2xl flex ${pathname === '/spend' ? 'bg-gray-300 border-b-2 border-green-600 text- rounded-2xl':''} `}> 
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
...
<Link className="pl-3" href="/spend">spend</Link></li>: null}
      ...
      {role.role === 'admin' ?   <li className={`p-1 m-2  justify-center text-center rounded-2xl flex 
...
<Link className="pl-3" href='/create'>Create</Link></li> : null}
       ...
       {role.role === 'admin' || role.role === 'editor' ?  <li className={`p-1 m-2  justify-center text-center rounded-2xl flex ${pathname === '/program' ? 'bg-gray-300 border-b-2 border-green-600 text- rounded-2xl':''} `}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
...
<Link className="pl-3" href='/program'>program</Link></li>:null}
     ...
     {role.role === 'admin' || role.role === 'admistration' ?   <li className={`p-1 m-2  justify-center text-center rounded-2xl flex
...
<Link className="pl-3" href='/payment'>Payment</Link></li>: null}
      ...
      {role.role === 'admin' || role.role === 'editor' || role.role === 'prof' ? <li className={`p-1 m-2  justify-center text-center rounded-2xl flex
