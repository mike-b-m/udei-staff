...
{hover ? 'home' :null}</li></Link> :null}
       ...
       {role.role === 'admin' || role.role === 'editor' ?    <Link className="pl-3" href="/admin/inscription">
        <li className={`m-2 ${hover ? 'w-[90%]':null} text-center rounded-2xl flex
             ${pathname === '/admin/inscription' ? 'bg-[#0077B6] border-b-[4px] font-bold border-blue-200 text-white rounded-[8px]'
...
{hover ? 'Inscription' : null}</li></Link> : null}
       ...
       {role.role === 'admin' || role.role === 'editor' ?   <Link className="pl-3" href="/admin/search">
       <li className={`m-2 ${hover ? 'w-[90%]':null} text-center rounded-2xl flex ${pathname === '/admin/search' ? 'bg-[#0077B6] border-b-[4px] font-bold border-blue-200 text-white rounded-[8px]':'hover:bg-[#137737] hover:text-white rounded-[8px]'} `}>
...
</svg> {hover ? 'Recherche' : null} </li></Link> :null}
       
        {/*search section */}
        {role.role === 'admin' || role.role === 'admistration' ? <Link className="pl-3" href="/admin/spend">
        <li className={` m-2 ${hover ? 'w-[90%]':null}  text-center rounded-2xl flex ${pathname === '/admin/spend' ? 'bg-[#0077B6] border-b-[4px] font-bold border-blue-200 text-white rounded-[8px]':'hover:bg-[#137737] hover:text-white rounded-[8px]'} `}> 
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`size-8 pl-2 ${hover ? 'mr-2':null}`}>
  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" />
</svg>
       
{hover ? 'dépense' : null} </li></Link>: null}

      {role.role === 'admin' ?   <Link className="pl-3" href='/admin/create'>
      <li className={`m-2 ${hover ? 'w-[90%]':null}  text-center rounded-2xl flex 
            ${pathname === '/admin/create' ? 'bg-[#0077B6] border-b-[4px] font-bold border-blue-200 text-white rounded-[8px]':'hover:bg-[#137737] hover:text-white rounded-[8px]'} `}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`size-8 pl-2 ${hover ? 'mr-2':null}`}>
...
{hover ? 'Compte' : null}</li> </Link> : null}

       {role.role === 'admin' || role.role === 'editor' ?  <Link className="pl-3" href='/admin/program'>
       <li className={`m-2 ${hover ? 'w-[90%]':null}  text-center rounded-2xl flex ${pathname === '/admin/program' ? 'bg-[#0077B6] border-b-[4px] font-bold border-blue-200 text-white rounded-[8px]':'hover:bg-[#137737] hover:text-white rounded-[8px]'} `}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`size-8 pl-2 ${hover ? 'mr-2':null}`}>
...
{hover ? 'Programme' : null}</li></Link>:null}

     {role.role === 'admin' || role.role === 'admistration' ?   <Link className="pl-3" href='/admin/payment'>
     <li className={`m-2 ${hover ? 'w-[90%]':null}  text-center rounded-2xl flex
         ${pathname === '/admin/payment' ? 'bg-[#0077B6] border-b-[4px] font-bold border-blue-200 text-white rounded-[8px]':'hover:bg-[#137737] hover:text-white rounded-[8px]'} `}>
     <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`size-8 pl-2 ${hover ? 'mr-2':null}`}>
  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75
   ...
   .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0
    ...
    0 0 1 6 0Zm3 0h.008v.008H18V10.5Zm-12 0h.008v.008H6V10.5Z" />
      ...
      {role.role === 'admin' || role.role === 'editor' || role.role === 'prof' ? <Link className="pl-3" href='/admin/teacher'>
      <li className={`m-2 ${hover ? 'w-[90%]':null}  text-center rounded-2xl flex
         ${pathname === '/admin/teacher' ? 'bg-[#0077B6] border-b-[4px] font-bold border-blue-200 text-white rounded-[8px]':'hover:bg-[#137737] hover:text-white rounded-[8px]'} `}>
