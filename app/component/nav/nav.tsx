...
export default function Nav(){
    ...
    return(
        ...
        {/*link for site */}
         ...
         className={`border-r-2 ${hover ? 'w-50':null} border-b-2 border-gray-600 top-0 left-0 text-[20px] text-center
    ...
    <ul className=" h-[80%] ">
       ...
       <Link className="pl-3 " href="/"><li className={`m-2 ${hover ? 'w-[90%]':null} text-center rounded-2xl pl-3 
         ...
         :'hover:bg-[#137737] rounded-[8px]'} w-14`}>
...
{hover ? 'home' :null}</li></Link> :null}
       ...
       {role.role === 'admin' || role.role === 'editor' ?    <Link className="pl-3" href="/inscription">
        <li className={`m-2 ${hover ? 'w-[90%]':null} text-center rounded-2xl flex
             ...
             :'hover:bg-[#137737] rounded-[8px]'} w-13`}>
...
{hover ? 'Inscription' : null}</li></Link> : null}
       ...
       <li className={`m-2 ${hover ? 'w-[90%]':null} text-center rounded-2xl flex ${pathname === '/search' ? 'bg-[#2DAE0D] border-b-[4px] font-bold border-[#137737] text-white rounded-[8px]':'hover:bg-[#137737] rounded-[8px]'} `}>
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
        ...
        <li className={` m-2 ${hover ? 'w-[90%]':null}  text-center rounded-2xl flex ${pathname === '/spend' ? 'bg-[#2DAE0D] border-b-[4px] font-bold border-[#137737] text-white rounded-[8px]':'hover:bg-[#137737] rounded-[8px]'} `}> 
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`size-8 pl-2 ${hover ? 'mr-2':null}`}>
...
{hover ? 'Create' : null}</li> </Link> : null}
       ...
       <li className={`m-2 ${hover ? 'w-[90%]':null}  text-center rounded-2xl flex ${pathname === '/program' ? 'bg-[#2DAE0D] border-b-[4px] font-bold border-[#137737] text-white rounded-[8px]':'hover:bg-[#137737] rounded-[8px]'} `}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`size-8 pl-2 ${hover ? 'mr-2':null}`}>
